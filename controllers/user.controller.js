      ...
      db.query(check_user_exists_query, [email], (err, result) => {
        ...
        if (organization_id) {
          ...
          db.query(fetch_org_id, [organization_id], (err, result) => {
            ...
            db.query(role_check, [user_role_id], (err, result) => {
              if (err) {
      ...
      db.beginTransaction((err) => {
        ...
        db.query(hr_check, [admin_id, organization_id], (err, result) => {
          if (err) {
            ...
            return db.rollback(() => {
              return res.status(500).json({ message: "Error checking hr exists" });
            });
          }
          if (result.length === 0) {
            return db.rollback(() => {
          ...
          db.query(user_check, [user_id, organization_id], (err, result) => {
            ...
            db.query(role_check, [new_role_id], (err, result) => {
              ...
              const previous_role_check =
                "SELECT * from apt_user_roles where user_id = ? and id = ?";
              db.query(
                previous_role_check,
                [user_id, previous_role_id],
                (err, result) => {
                  if (err) {
                    console.error("Error checking previous role exists: ", err);
                    return db.rollback(() => {
                      return res.status(500).json({
...
// Delete Users :: It Will Handle By HR Only
    ...
    db.beginTransaction((err) => {
      ...
      db.query(hr_check, [hr_id, normalizedOrganizationId], (err, result) => {
        ...
        db.query(hr_val, [hr_id], (err, result) => {
          ...
          db.query(deleting_users_details, [normalizedUserIds], (err, result) => {
            if (err) {
